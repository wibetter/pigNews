...
export const formatDate = (timeStamp, format = '/', flag = true) => {
  ...
  return flag
    ? `${timeObj.year}${format}${doubleNum(timeObj.month)}${format}${doubleNum(
        timeObj.day,
      )}`
...
export const isPicture = (curImg) => {
  ...
  if (curImg && curImg.indexOf('.') > -1) {
    const startIndex = curImg.lastIndexOf('.');
    let endIndex = curImg.length;
    if (curImg.indexOf('?') > -1) {
      endIndex = curImg.lastIndexOf('?');
    }
    let strPostfix = curImg.substring(startIndex, endIndex);
