<template>
  <div class="smooth-picker flex-box">
    ...
    <div
      ref="smoothGroup"
      v-for="(group, gIndex) in dataList"
      :key="gIndex"
      class="smooth-group"
...
export default {
  ...
  data() {
    return {
      ...
      groupsRectList: new Array(this.dataList.length),
  ...
  methods: {
    ...
    setCurrentIndexOnMove(ev) {
      ...
      if (
        typeof gIndex === 'number' &&
        (this.dataList[gIndex].divider || !this.dataList[gIndex].list)
      ) {
        return;
      }
    ...
    correctionCurrentIndex(ev, gIndex) {
      setTimeout(() => {
        if (
          typeof gIndex === 'number' &&
          this.dataList[gIndex].divider !== true &&
          this.dataList[gIndex].list.length > 0
        ) {
    ...
    getItemStyle(gIndex, iIndex) {
      ...
      if (Math.abs(gapCount) < 4) {
        let rotateStyle = `transform: rotateX(${
          gapCount * 23
