/* eslint-disable */
var Alert = require('./toast.vue').default; // 引入vue模板
var Toast = {}; // 定义插件对象
Toast.install = function (Vue, options) {
  // vue的install方法，用于定义vue插件
  // 如果toast还在，则不再执行
  if (window.document.getElementsByClassName('alertBox').length) {
    return;
  }
    ...
    show(options) {
      ...
      if (!$vm.show) {
        if (typeof options === 'string') {
          ...
          Object.assign($vm, newOptions); // 合并参数与实例
        }
...
// 导出Toast（注意：此处不能用module exports导出，在一个文件中，不能同时使用require方式引入，而用module exports导出，两种方式不能混用）
